...
FR-016 to FR-021), UC-007, `docs/entity_model.md`, `research/Kimball_type_2.md`,
...
| `respondents.lang` | Dropped | Nothing needs it, and UC-007 BR-007 says the language choice is session-only. |
| Review as a view with `_en/_es/_ar` columns | Cannot be ported | Languages are open-ended (mounted overlays, FR-018). The review SQL takes `:language` and joins the translations table. |
...
content translation is tracked on a separate branch." UC-007 BR-005 says content keeps
...
the next visit is what UC-007 BR-007 forbids. BR-007 should be amended to say the choice
...
which computes `source_hash` and `source_text` from the current base text. A CSV export
of missing and stale strings and a CSV import fit the existing hand-off pattern
(FR-020, FR-046) as a later step.
...
language"), a new FR for localized survey content under UC-007, and an NFR on fallback.
UC-007: BR-005 becomes "content appears in the respondent's language when a current
...
Author: C-021 rewritten; C-015 names the redefined record set; a new FR and use case
"Translate survey content" and an FR "Manage content languages" (plus
`use_cases.puml`); NFR-011 mentions the `translations` record; FR-036 cited as the
